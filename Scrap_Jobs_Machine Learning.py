...
def Scrap_Machine_Learning():
    Title_lst, links_list, occupations_lst, Companies_lst, Specs_lst, Location_lst = [
    ], [], [], [], [], []
    URL = "https://wuzzuf.net/search/jobs/?a=hpb%7Cspbg&q=Machine%20Learning"
    ...
    for i in range(MaxPage):
        RES = requests.get(
            "https://wuzzuf.net/search/jobs/?a=hpb%7Cspbg&q=Machine%20Learning&start="+str(i))
    ...
    df.to_csv("N_Scrap_Machine Learning1.csv", index=True)
    print("Jobs Scraped Sucsessfully (Machine Learning)")
    return df


if __name__ == "__main__":
    Scrap_Machine_Learning()
